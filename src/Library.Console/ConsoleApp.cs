...
public class ConsoleApp
{
    ...
    async Task<ConsoleState> SearchBooks()
    {
        ...
        var foundBook = _jsonData.Books?.FirstOrDefault(b => !string.IsNullOrWhiteSpace(b.Title) && b.Title.Contains(bookTitle!, StringComparison.OrdinalIgnoreCase));

        if (foundBook == null)
        {
            Console.WriteLine($"No book found with title matching: {bookTitle}");
            return ConsoleState.PatronDetails;
        }
        ...
        {
            Console.WriteLine($"{foundBook.Title} has no copies in inventory.");
            return ConsoleState.PatronDetails;
        }

        var bookItemIds = bookItems.Select(bi => bi.Id).ToHashSet();
        var activeLoan = _jsonData.Loans?.FirstOrDefault(l => bookItemIds.Contains(l.BookItemId) && l.ReturnDate == null);
